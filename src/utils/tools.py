...
def filter_stop_word(sens):
    stopwords = [
        x.strip('\n') for x in open(root_path + '\\data\\stopwords.txt', encoding='utf-8').readlines()
    ]
    sens = [x for x in sens if x not in stopwords]
    return sens
...
def create_logger(log_path):
    ...
    ch.setLevel(level_relations.get('info'))
    # add formatter to ch
    ...
    # sh.setLevel(level_relations.get('info'))
    # sh.setFormatter(format_str)  # 设置屏幕上显示的格式
    th = handlers.TimedRotatingFileHandler(
        filename=log_path, when='D', backupCount=3,
        encoding='utf-8')  # 往文件里写入#指定间隔时间自动生成文件的处理器
    th.setLevel(level_relations.get('info'))
