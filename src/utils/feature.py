...
from src.utils.tools import wam, format_data, query_cut
...
def get_pretrain_embedding(text, tokenizer, model):
    ...
    return res[1].detach().cpu().numpy()[0]
...
def get_embedding_feature(data, tfidf, embedding_model, embedding_model_2):
    '''
    ...
    for key in labelIndexToName:
        if labelIndexToName[key] in embedding_model.wv.vocab.keys():
            w2v_label_embedding.append(embedding_model.wv.get_vector(labelIndexToName[key]))
        else:
            # 因为类别中含有停用词，所以要把类别中的停用词剔除，不然会出现oov问题
            book_label = re.sub("[与/]", "", labelIndexToName[key])
            w2v_label_embedding.append(embedding_model_2.wv[book_label])
    # w2v_label_embedding = np.array([
    #     embedding_model.wv.get_vector(labelIndexToName[key])
    #     for key in labelIndexToName
    #     if labelIndexToName[key] in embedding_model.wv.vocab.keys()
    # ])
    w2v_label_embedding = np.array(w2v_label_embedding)
    joblib.dump(w2v_label_embedding,
                config.root_path + '/data/w2v_label_embedding.pkl')
    # joblib.dump(np.array(w2v_label_embedding),
    #             config.root_path + '/data/Debug_w2v_label_embedding.pkl')
...
def tag_part_of_speech(data):
    ...
    noun_count = len(
        [w for w in words if w[1] in ('n', 'nr', 'ns', 'nt', 'nw')])
    adjective_count = len([w for w in words if w[1] in ('a', 'ad', 'an')])
    verb_count = len([
        w for w in words if w[1] in ('v', 'vd', 'vn')
    ])
...
def get_basic_feature(df):
    ...
    # 词的个数
    df['num_words'] = df['queryCutRMStopWord'].progress_apply(lambda x: len(x))
    # 唯一词的个数
    df['num_unique_words'] = df['queryCutRMStopWord'].progress_apply(
...
def Find_Label_embedding(example_matrix, label_embedding, method='mean'):
    ...
    # 将样本的词嵌入与注意力机制相乘得到
    attention_embedding = np.dot(attention, example_matrix)
